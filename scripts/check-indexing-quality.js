...
function hasArticleModifiedTime(html, value) {
  ...
  return tags.some((tag) =>
    ...
    new RegExp(`\\bcontent=["']${escaped}["']`, 'i').test(tag)
  )
}
...
const expectedNoindexPaths = new Set([
  ...
  '/guides/recipe-planning-route/',
  ...guides
    .filter((item) => isEditorialContent(item.data_status) || !isIndexableGuide(item))
    .map((item) => `/guides/${item.slug}/`),
...
if (fs.existsSync(sitemapPath)) {
  ...
  for (const [, url, lastModified] of sitemapEntries) {
    ...
    if (/^\/guides\/[^/]+\/$/.test(pagePath)) {
      assert(indexableGuidePaths.has(pagePath), `sitemap includes guide without complete review data: ${pagePath}`)
    }
...
for (const item of news.filter((entry) => entry.updated_at)) {
  ...
  assert(
    sitemapDate === expectedDate,
    `news sitemap lastmod does not match updated_at for ${pagePath}: expected ${expectedDate}, received ${sitemapDate || 'missing'}`
  )

  const htmlFile = htmlPathForRoute(pagePath)
  if (fs.existsSync(htmlFile)) {
    const head = fs.readFileSync(htmlFile, 'utf8').slice(0, 80000)
    assert(hasArticleModifiedTime(head, expectedDate), `news page is missing matching article:modified_time metadata for ${pagePath}`)
  }
}
...
for (const item of searchIndex) {
  ...
  if (item.type === 'Guide' && pagePath !== '/guides/') {
    assert(indexableGuidePaths.has(pagePath), `search index includes guide without complete review data: ${pagePath}`)
  }
}
...
assert(
  guideEntries.every((item) =>
    item.status === 'Source-backed guide' ||
    item.status === 'Reviewed guide' ||
    item.status === 'Editorial guide' ||
    item.status === 'Unverified editorial guide' ||
    item.status === 'Source-backed guide hub'
  ),
